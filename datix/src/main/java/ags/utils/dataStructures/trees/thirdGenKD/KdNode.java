...
class KdNode<T> {
    ...
    public double[] addPoint(double[] point, T value) {
        KdNode<T> cursor = this;
        while (!cursor.isLeaf()) {
    ...
    public double[] addLeafPoint(double[] point, T value) {
        // Add the data point
        points[size] = point;	
        data[size] = value;
        extendBounds(point);
    ...
    private double[] splitLeafNode() {
        ...
        points = null;
        data = null;
        double[] idz = new double[]{id, nextId, nextId+1, splitValue, splitDimension};
        return idz;
    }
